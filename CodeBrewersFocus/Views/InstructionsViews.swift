...
// MARK: - InstructionView (OnboardingView with integrated media picker and player)
    ...
    var body: some View {
        TabView(selection: $currentPage) {
            ForEach(pages.indices, id: \.self) { index in
                VStack(spacing: 20) {
                    // Title and description of each onboarding page
                    ...
                    // Media section: show GIF or looping video, or fallback placeholder
                    ...
                    HStack(spacing: 8) {
                        ForEach(pages.indices, id: \.self) { dotIndex in
                            Circle()
                                .fill(dotIndex == currentPage ? Color.black : Color.gray.opacity(0.4))
