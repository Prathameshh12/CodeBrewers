...
class PuzzlePiece: Identifiable, Hashable, Encodable, Decodable, Transferable, ObservableObject {
    var id = UUID()
    let imageName: String
    @Published var isInverted: Bool = false
    @Published var rotation: Double = 0
    @Published var flippedHorizontally: Bool = false
    @Published var flippedVertically: Bool = false
    ...
    required init(from decoder: Decoder) throws {
        ...
        flippedHorizontally = try container.decode(Bool.self, forKey: .flippedHorizontally)
        flippedVertically = try container.decode(Bool.self, forKey: .flippedVertically)
        isPlaceholder = try container.decode(Bool.self, forKey: .isPlaceholder)
    }
    ...
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: 14)
                .fill(piece.isInverted ? Color.black : Color(white: 0.9))
            Image(piece.imageName)
                .resizable()
                .foregroundStyle(piece.isInverted ? Color(white: 0.9) : Color.black)
...
#Preview {
//    PuzzleView(path: .constant([""]))
//        .environmentObject(PuzzleSession())
    MainTabView()
}
